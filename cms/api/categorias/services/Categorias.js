...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Categorias.query(function(qb) {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Categorias._attributes)
      .filter(attribute => attribute !== Categorias.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Categorias._attributes[attribute].type));
